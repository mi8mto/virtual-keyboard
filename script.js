...
const addValue = (object, value) => {
  ...
  if (i === j) {
    obj.textArea.value =
      obj.textArea.value.substring(0, i) +
      value +
      obj.textArea.value.substring(i);
  } else {
    obj.textArea.value =
      obj.textArea.value.substring(0, i) +
      value +
...
const startP = () => {
  ...
  TextTitle.innerHTML = "RSS VIRTUAL KEYBOARD";
  switchLang.innerHTML = `
  <p class="p_lang">Change language - Left Shift + Left Alt </p>
`;
  wrapper.append(TextTitle, switchLang, textArea, keyBoardBlock);
...
class Keyboard {
  ...
  init(keys) {
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      keys[i].forEach((item) => {
        ...
        btn.innerHTML = this.capsButtons(item);
        btn.setAttribute("data-code", this.keys[item].code);
        if (this.keys[item].code === "CapsLock" && this.properties.CapsLock) {
          btn.classList.add("green_bg");
        } else if (
          this.keys[item].code === "ShiftLeft" &&
          this.properties.ShiftLeft
        ) {
          btn.classList.add("green_bg");
        } else if (
          this.keys[item].code === "ShiftRight" &&
          this.properties.ShiftRight
        ) {
          btn.classList.add("green_bg");
        } else if (
          this.keys[item].code === "AltLeft" &&
          this.properties.altOn
        ) {
          btn.classList.add("green_bg");
        }
  ...
  capsButtons(key) {
    if (
      ...
      !this.keys[key].functional &&
      this.keys[key].code !== "Space" &&
      !this.properties.CapsLock
    ) {
      if (this.properties.language === "EN" && this.keys[key].shiftEN)
        return this.keys[key].shiftEN;
      if (this.properties.language === "RU" && this.keys[key].shiftRU)
        return this.keys[key].shiftRU;
    ...
    document.querySelector(".wrapper").addEventListener("click", (e) => {
      if (e.target.classList.contains("btn")) {
        if (e.target.dataset.code === "Tab") {
          ...
          !this.keys[e.target.dataset.code].functional &&
          !e.target.dataset.data === "Space" &&
          (this.properties.ShiftRight || this.properties.ShiftLeft)
        ) {
...
document.body.addEventListener("keydown", (e) => {
  ...
  if (condition) {
    ...
    btn.classList.add("green_bg");
    if (
      keyboard.keys[e.code].code === "Tab" ||
      keyboard.keys[e.code].EN === "Alt"
    )
      e.preventDefault();
...
document.body.addEventListener("keyup", (e) => {
  ...
  if (condition) {
    ...
    if (
      keyboard.keys[e.code].code !== "ShiftLeft" &&
      keyboard.keys[e.code].code !== "ShiftRight"
    ) {
      document
